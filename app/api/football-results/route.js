export async function GET() {
  try {
      ...
      if (response.status === 429) {
        return Response.json(
          { 
            ...
            message: 'Достигнат е лимитът на безплатните заявки. Моля, опитайте по-късно или се свържете с администратора.'
          },
          { status: 429 }
        );
      }
    ...
    return Response.json(
      { 
        error: 'NETWORK_ERROR',
        message: 'Грешка при зареждане на футболни резултати. Моля, проверете интернет връзката.'
      },
      { status: 500 }
    );
  }
